...
export class MockObsDialogComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.size[0] > this.size[1]) {
      this.container.nativeElement.style.transform = `scale(${ratio}) translateY(${
        (rect.height - ratio * this.size[1]) / ratio / 2
      }px)`;
      this.container.nativeElement.style.transformOrigin = `left top`;
    } else {
      this.container.nativeElement.style.transform = `scale(${ratio2}) translateX(${
        (rect.width - ratio2 * this.size[0]) / ratio2 / 2
      }px)`;
      this.container.nativeElement.style.transformOrigin = `left top`;
    }
  }
