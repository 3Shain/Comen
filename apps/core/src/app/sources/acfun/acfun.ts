...
import { Message, StickerMessage, TextMessage } from '@comen/common';
...
export class AcfunSource {
  ...
  connect({ roomId }) {
    return this.http
      .get<AcfunRoominfoResponse>(`/api/acfun/getRoomInfo?roomid=${roomId}`)
      .pipe(
        switchMap((data) => {
          if (data.closed) {
            // TODO:
              ...
              switch (msg.type) {
                case 'CommonActionSignalComment':
                  observer.next({
                    ...
                    usertype:
                      msg.data.userInfo.userIdentity.managerType == 1 ? 0x2 : 0,
                ...
                case 'CommonActionSignalGift': {
                  ...
                  observer.next({
                    ...
                    platformPrice:
                      giftInfo.payWalletType == 2 ? giftInfo.giftPrice : 0,
