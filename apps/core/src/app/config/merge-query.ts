...
export function mergeQueryParameters(
  query: SafeAny,
  config: ComenConfiguration
) {
  if (query.bilichat != undefined) {
    ...
    config.groupSimilarWindow =
      'groupSimilarWindow' in query ? parseInt(query.groupSimilarWindow) : 5;
    ...
    if (query.blackList != undefined) {
      const blacklist = (query.blackList as string)
        .split(',')
        .map((v) => parseInt(v));
