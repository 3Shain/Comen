...
export class BilibiliUserService {
  ...
  async fetchUserInfo(uid: number): Promise<CachedBilibiliUserInfo> {
    ...
    while (true) {
      if (
        Date.now() - this.lastFetch < MINIMUM_FETCH_INTERVAL ||
        this.interfaceUnlock > Date.now()
      ) {
        ...
        if (this.currentFetchJob == null) {
          this.currentFetchJob = setTimeout(async () => {
            ...
            const ids = [...this.fetchQueue].slice(
              0,
              this.fetchQueue.size > 20 ? 20 : this.fetchQueue.size
            ); // maximum size?
            ...
            try {
              ...
              const ret = (
                await got.get<{
                  ...
                  `https://api.live.bilibili.com/user/v3/User/getMultiple?${ids
                    .map((x) => `uids[]=${x}`)
                    .join('&')}&attributes[]=info`,
                  {
                    headers: {
                      'User-Agent': '',
                    },
  ...
  async getUserInfoFromCache(uid: number): Promise<CachedBilibiliUserInfo> {
    const info = (await this.cache.get(
      `BILI_USERINFO_${uid}`
    )) as CachedBilibiliUserInfo | null;
    if (info) {
