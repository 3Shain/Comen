...
const baseCommonActionSignalLike: object = {
  sendTimeMs: Long.ZERO,
};

export const protobufPackage = 'AcFunDanmu'

export const CommonActionSignalLike = {
  encode(message: CommonActionSignalLike, writer: Writer = Writer.create()): Writer {
  ...
  decode(input: Uint8Array | Reader, length?: number): CommonActionSignalLike {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseCommonActionSignalLike } as CommonActionSignalLike;
  ...
  fromJSON(object: any): CommonActionSignalLike {
    const message = { ...baseCommonActionSignalLike } as CommonActionSignalLike;
  ...
  fromPartial(object: DeepPartial<CommonActionSignalLike>): CommonActionSignalLike {
    const message = { ...baseCommonActionSignalLike } as CommonActionSignalLike;
    if (object.userInfo !== undefined && object.userInfo !== null) {
  ...
  toJSON(message: CommonActionSignalLike): unknown {
    const obj: any = {};
