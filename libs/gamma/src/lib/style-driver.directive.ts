...
export class StyleDriverDirective implements OnInit, OnDestroy {
    ...
    constructor(
        private host: ComenEnvironmentHost,
    ...
    ngOnInit() {
        (this.context != null
            ? this.host
                  .variantPipe(this.configSection)
                  .pipe(map((fn) => fn(this.context)))
            ...
            .subscribe((config) => {
                if (config.font) {
                    ...
                    this.e.style.textDecoration = `${
                        config.font.underline ? 'underline' : ''
                    } ${
                        config.font.strikeThrough ? 'line-through' : ''
                    }`.trim();
                } else {
...
function textShadow(x: any, y: any, color: string) {
